...
def evaluate(video, output_name, net, multiscale=False, visualize=False):
    ...
    for sample in tqdm(dataset):
        ...
        if visualize:
            for keypoints in coco_keypoints:
                ...
                for idx in range(len(keypoints) // 3):
                    if idx in [5, 9]: # 0:nose 1: 눈 2: 눈 3: 귀 4: ? 5:왼쪽 어깨 6: 오른쪽 어깨 7: 왼쪽 팔꿈치 8: 9:왼쪽 손목 
        ...
        #keys.append(list(coco_keypoints[0][15],coco_keypoints[0][16],coco_keypoints[0][27],coco_keypoints[0][28]))
    ...
    # run_coco_eval(img_list, output_name)
...
def model():
    ...
    video = "/Users/islandz/Documents/Ewha/Capstone/newdata/150f.mov"
    net = PoseEstimationWithMobileNet()
    checkpoint = torch.load("weight.pth", map_location=torch.device('cpu'))
    ...
    return {"rom":max_angle, "speed":speed}
