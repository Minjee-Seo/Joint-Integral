...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///integral.db' #가상의 db생성 
...
class JIresult(db.Model): 
    ...
    def __init__(self, date, time, rom, jp, height):
        self.date = date
        self.time = time
        self.rom = rom
        self.jp = jp
...
def run():
    if request.method == 'POST':
        ...
        data = JIresult(str(now.date()), str(now.time()), max_angle, jp, float(height))
